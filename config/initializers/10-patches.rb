...
module ActiveRecord
  class Base
    ...
    # Make class const to be a hash
    # Eg:
    # Version::VERSION_COMPILE_STATUS[:submitted] #=> Version.consts[:compile_status][:submitted]
    def self.consts
      constants.inject({}) do |r, const|
        if /#{name.underscore}_/i === const.to_s
          const_key = $'.downcase.to_sym
          r[const_key] = const_get const
        end
        r
      end
    end
