class IssueToApprovesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html {
        @status = IssueToApproveTask::ASSIGNED_STATUS.to_a.map { |status| {:name => status[1][1], :id => status[1][0]} }
        @repos = @project.repo_requests.success_requests
        @limit = per_page_option

        scope = IssueToApproveMerge.approves
