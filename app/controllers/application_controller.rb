...
class ApplicationController < ActionController::Base
  ...
  helper_method :current_user, :bool_from
  ...
  def logout_user
    ...
    # CASClient::Frameworks::Rails::Filter.logout(self, CASClient::Frameworks::Rails::Filter.config[:service])
    base_url = CASClient::Frameworks::Rails::Filter.config[:cas_base_url]
    service = CGI.escape signin_url.gsub(/http:\/\/|https:\/\//,'')
    redirect_to "#{base_url}/logout?redirect=#{service}"
  end
  ...
  alias auth authorize # Alias pundit authorize
  alias skip_auth skip_authorization
  ...
  def render_error(arg=nil)
    ...
    respond_to do |format|
      format.html {
        ...
        render :template => 'exception_handler/exceptions/show', :layout => use_layout, :status => @status
      }
  ...
  def check_user_if_outsource
    redirect_to home_url if !User.current.logged? || User.current.firstname.to_s.include?("外包")
  end

  def remember_forwarding_url
    session[:forwarding_url] = url_for(params) if controller_name != "account"
  end

  def set_no_cache
    response.headers["Cache-Control"] = "no-cache, no-store, max-age=0, must-revalidate"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "Sat, 26 Dec 1992 00:00:00 GMT"
  end
    ...
    workbook.add_worksheet(name: "GIONEE") do |sheet|
      ...
      items.each do |item|
        sheet.add_row (columns.keys.map { |c|
          item.class == Hash ?
              (c.to_s.end_with?('_dt') || c.to_s.end_with?('_at') || c.to_s.end_with?('_on') ? format_date(item[c.to_s.to_sym]) : item[c.to_s.to_sym].to_s) :
                  (c.to_s.end_with?('_dt') || c.to_s.end_with?('_at') || c.to_s.end_with?('_on') ? format_date(item.send(c)) : item.send(c).to_s) }), style: body
      end
    end
  ...
  # call mysql Stored Procs
  def call_stored_procs(procs_name, args = [])
    arbase = ActiveRecord::Base
    query_result = arbase.connection.exec_query("call #{procs_name}(#{args.join(',')})")
    arbase.clear_active_connections!
    query_result.map{|res| res}
  end
  ...
  def remove_job_from_jobset(jobset)
    case jobset
      when "retryset"
        Sidekiq::RetrySet
      when "deadset"
        Sidekiq::DeadSet
    end.new.select { |set| set }.map(&:delete)
  end

  def delete_job_from_queue(queue_name, args)
    Sidekiq::Queue.new(queue_name).select { |q| JSON.parse(q.value)["args"].first["arguments"] == [args] }.delete
  end
