...
class AttachmentsController < ApplicationController
  ...
  before_action :set_no_cache, :only => :download
  ...
  def upload
    ...
    @attachment = Attachment.find_or_initialize_by(:uniq_key => params[:uniq_key])
    @attachment.file = request.raw_post
    @attachment.author = User.current
    @attachment.filename = params[:name].presence || Redmine::Utils.random_hex(16)
    @attachment.filesize = params[:size].presence
    @attachment.content_type = params[:type].presence
    @attachment.chunk = params[:chunk].presence
    @attachment.extra_type = params[:extra_type].presence

    if @attachment.new_record? #50MB
    ...
    respond_to do |format|
      format.js { render json: {:token => "#{@attachment.id}.#{@attachment.uniq_key}"} }
      format.api {
        if saved
