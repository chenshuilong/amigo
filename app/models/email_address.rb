...
class EmailAddress < ActiveRecord::Base
  ...
  scope :get_email_name, lambda { |users| where(user_id: users, is_default: true)
                                          .where("address like '%gionee.com%'")
                                          .map{|ea| ea.address.gsub('@gionee.com', '')}}
  ...
  def downcase_address
    self.address = address.downcase unless self.address.blank?
  end
