...
class Mailer < ActionMailer::Base
  ...
  def resigned_notification(recipients, options={})
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    @bugs = options[:bugs]
    ...
    mail :to => recipients,
      :subject => "[#{Setting.app_title}] #{l(:mail_subject_resigned_notification)}"
  end


  def undisposed_bugs_notification(recipients, options={})
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    @type = options[:type]
    ...
    # recipients = User.where(:id => 1125)
    mail :to => recipients,
      :subject => "[#{Setting.app_title}] #{l(:mail_subject_undisposed_bugs_notification)}"
  end


  def apply_umpirage_notification(recipients, options={})
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    ...
    @user = options[:user]
    mail :to => recipients,
      :subject => "[#{Setting.app_title}] #{l(:mail_subject_apply_umpirage_notification)}"
  end

  def version_compiled_notification(recipients, options={})
    @version = options[:version]
    @project = @version.project
    @issues    = @version.issues
    @app_lists = @version.app_lists

    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    ota = @version.ota_whole_compile?? "[OTA]" : nil
    subject = if @version.compiled_successfully?
      "[#{@version.fullname}]#{ota} #{l(:mail_subject_version_compiled_success)}"
    else
      "[#{@version.fullname}]#{ota} #{l(:mail_subject_version_compiled_failed)}"
    end
    mail :to => recipients, :subject => subject
  end

  def version_released_notification(recipients, options={})
    @release = options[:release]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    mail :to => recipients, :subject => "[#{@release.version.try(:fullname)}] #{l(:mail_subject_version_released)}"
  end

  def version_release_flow_notification(recipients, options={})
    @release = options[:release]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    mail :to => recipients, :subject => "[#{@release.version.try(:fullname)}] #{l(:mail_subject_version_release_flow_notification)}"
  end

  def issue_to_merged_notification(recipients, options={})
    @issue = options[:issue_to_merge]
    @success = options[:success]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    mail :to => recipients, :subject => "[#{@issue.issue.try(:name)}] #{l(:mail_subject_issue_to_merged)}"
  end

  def personal_task_notification(recipients, options={})
    @task = options[:task]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    mail :to => recipients, :subject => "[#{l(:label_personal_task)}] #{@task.name}"
  end

  def library_failed_notification(recipients, options={})
    @library = options[:library]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    mail :to => recipients, :subject => "[#{l('library_status_'+@library.status)}] 库:#{@library.name} "
  end

  def invalid_apk_base_notification(recipients, options={})
    @version = options[:version]
    @project = options[:project]
    @invalid_apks = options[:invalid_apks]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    if @project.category.to_i == 4
      subject = "[#{@version.fullname}]版本新增APK信息如下，请到#{@project.name}产品“APK基本信息”模块确认处理"
    else
      subject = "[#{@version.fullname}]版本中如下APK信息未录入阿米哥系统，请确认！"
    end
    mail :to => recipients, :subject => subject
  end

  def apk_base_approve_notification(recipients, options={})
    @apk_base = options[:apk_base]
    @project = options[:project] if options[:project].present?
    @apk_base_info = options[:apk_base_info]
    redmine_headers 'Sender' => User.current.login
    @user = Array(recipients).detect{|r| r.is_a? User }
    set_language_if_valid(@user.try :language)
    if @apk_base.app_category == 1
      subject = "[阿米哥APK信息评审]#{@project.name}产品#{@apk_base.name}信息待评审："
    elsif @apk_base.app_category == 10
      subject = "[阿米哥Google原生APK信息评审]#{@apk_base.name}信息待评审："
    end
    mail :to => recipients, :subject => subject
  end
