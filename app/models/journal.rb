...
class Journal < ActiveRecord::Base
  ...
  scope :notes, -> {where.not(:notes => "")}

  scope :with_between_time, lambda {|bdt,edt|
    select("users.firstname as username,users.orgNo,users.orgNm,journals.*").joins(:user).
        where("#{table_name}.created_on >= '#{format_time(edt)}' and #{table_name}.created_on <= '#{format_time(bdt)}'")
  }
  ...
  def old_status
    s = detail_for_attribute('status_id').try(:old_value)
    s ? IssueStatus.find_by_id(s.to_i) : nil
  end
