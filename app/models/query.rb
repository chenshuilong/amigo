...
class Query < ActiveRecord::Base
  ...
  def hidden_columns
    columns.select{|c| [:status_histories].include? c.name}
  end
  ...
  def statement
    unless filters[:condition].blank?
      condition = filters[:condition]
      condition = condition.gsub('("me")', "(#{User.current.id.to_s})") # Replace "me"
      projects = User.current.projects.pluck(:id).join(",")  # Replace "mine"
      projects = "NULL" if projects.blank?
      condition = condition.gsub('("mine")', "(#{projects})")
      condition = condition.gsub(/\"dept[^\"]+\"/) do |dept| # Replace Department users
        user_ids= [19921226] # Prevent none id error
        dept.scan(/\d+/).each { |f| user_ids += Dept.find_by(:id => f).all_users.pluck(:id)}
        "(%s)" % user_ids.uniq.join(",")
      end
      condition = condition.gsub(/\"group[^\"]+\"/) do |group| # Replace group users
        user_ids = [19921226] # Prevent none user in group
        group.scan(/\d+/).each { |f| user_ids += Group.givable.visible.find_by(:id => f).users.pluck(:id)}
        "(%s)" % user_ids.uniq.join(",")
      end
    end
