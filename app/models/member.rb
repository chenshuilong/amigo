...
class Member < ActiveRecord::Base
  ...
  scope :users_role_project, lambda{ |project_id, role_name|
    joins("LEFT JOIN projects ON projects.id = members.project_id
    ...
    .where("roles.name = '#{role_name}' AND projects.id = #{project_id}")
  }
  ...
  def children
    MemberRole.where(:inherited_from => self.member_roles.pluck(:id)).map(&:member).uniq
  end
