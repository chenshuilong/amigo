...
class MemberRole < ActiveRecord::Base
  ...
  validates_uniqueness_of :role_id, :scope => :member_id
  attr_protected :id
  ...
  def add_role_to_group_users
    if member.principal.is_a?(Group) && !inherited?
      member.principal.users.each do |user|
        ...
        user_member_role = MemberRole.find_by(role_id: role, member_id: user_member.id)
        next if user_member_role.present? 
