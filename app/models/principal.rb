...
class Principal < ActiveRecord::Base
  ...
  scope :like, lambda {|q|
    ...
    if q.blank?
      ...
      pattern = "%#{q}%"
      sql = %w(login firstname lastname pinyin).map {|column| "LOWER(#{table_name}.#{column}) LIKE LOWER(:p)"}.join(" OR ")
  ...
  scope :without_member_of_role, lambda {|projects, role|
    projects = [projects] unless projects.is_a?(Array)
    if projects.empty?
      where("1=0")
    else
      ids = projects.map(&:id)
      where("#{Principal.table_name}.id NOT IN (SELECT DISTINCT user_id FROM #{Member.table_name} INNER JOIN member_roles ON member_roles.member_id = members.id WHERE project_id IN (?) AND member_roles.role_id = #{role})", ids)
    end
  }
  ...
  def productions
    projects.present?? projects.where(:category => Project::CATEGORY_APP) : Project.none
  end

  def word_avatar
    name.gsub(/\s|\(.+\)/, "").try(:last)
  end
