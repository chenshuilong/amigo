...
module IssuesHelper
  ...
  def issue_heading(issue)
    h("#{issue.tracker} ##{issue.id} -- #{issue.status.name}")
  end
  ...
  def rom_versions_options_for_select(project=nil)
    if project.nil?
      versions = Project.rom_versions
      versions = versions.zip versions
    else
      versions = project.rom_version.present? ?  project.rom_version.split : []
      versions = versions.zip versions
    end
  end
  ...
  class IssueFieldsRows
    ...
    def to_html
      content =
        content_tag('div', @left.reduce(&:+), :class => 'splitcontentleft col-md-6') +
        content_tag('div', @right.reduce(&:+), :class => 'splitcontentright col-md-6') +
        content_tag('div', nil, :class => 'clearfix') +
        content_tag('div', @line.reduce(&:+), :class => 'splitcontentline')
  ...
  class FieldsTable
    ...
    def initialize(options)
      ...
      @row = []
      @data = []
      @table_attr = options.merge({:class => "#{options[:class]} field_table"})
    end
    ...
    def head(text, options={})
      text = l(text) if text.is_a?(Symbol)
      content_tag('th', text, options)
    end

    def body(options={})
      content_tag('tr', @data.reduce(&:+), options)
    end

    def data(text, options={})
      @data << content_tag('td', text, options)
    end

    def to_html
      content =
        content_tag('thead', content_tag('tr', @title.reduce(&:+))) +
        content_tag('tbody', @row.reduce(&:+))
      content_tag('table', content, @table_attr)
    end
  end
  ...
  def show_detail(detail, no_html=false, options={})
    ...
    when 'attr'
      ...
      when 'project_id', 'status_id', 'tracker_id', 'assigned_to_id',
            'priority_id', 'category_id', 'fixed_version_id', 'tfde_id',
            'app_version_id', 'integration_version_id'
...
#####################
  ...
  def condition_star_list
    return "" unless User.current.logged?
    root = User.current.conditions.star.root
    if root.present?
      parse_root(root)
    else
      ""
  ...
  def parse_root(root)
    content_tag(:ul) do
      root.map do |r|
        if r.condition_files.present?
          content_tag(:li, link_to(r.name.gsub('/',''), "javascript:;", target: r.id) + parse_root(r.condition_files), class: (r.is_folder? ? "isFolder" : nil)) if Condition.find(r.id).present?
        else
          # content_tag(:li, link_to(r.name, "javascript:;", target: r.id), class: (r.is_folder? ? "isFolder" : nil))
          content_tag(:li, link_to(r.name.gsub('/',''), "javascript:;", target: r.id), class: (r.is_folder? ? "isFolder" : nil)) if Condition.find(r.id).present?
        end
      end.join('').html_safe
    end
  end

  def condition_history_list
    return "" unless User.current.logged?
    history = User.current.condition_histories.first(10) # Latest 10 record
    if history.present?
      content_tag :ul do
        history.map do |h|
          concat content_tag(:li, link_to(h.condition.name, "javascript:;", target: h.from_id))
        end
      end
    else
      ""
  ...
  def condition_system_list
    return "" unless User.current.logged?
    root = Condition.system.root
    if root.present?
      parse_root(root)
    else
      ""
  ...
  ## Struct Json to Issue Filter Window <select>

  def condition_json_for_select(key)
    case key
    when /status_id/
      condition_json_template("list", IssueStatus.pluck(:id, :name))
    when /project_id/
      condition_json_template("list", Project.pluck(:id, :name))
    when /tracker_id/
      condition_json_template("list", Tracker.pluck(:id, :name))
    when /priority_id/
      condition_json_template("list", IssuePriority.pluck(:id, :name))
    when /author_id|watcher_id|assigned_to_id/
      users = [["me", "<< #{l(:label_me)} >>"]]
      Project.all.each do |p|
        users = users | p.users.pluck(:id, :firstname).reject{|r| r.blank?}
      end
      condition_json_template("user", users)
    when /is_private/
      condition_json_template("bool")
    when /cf_/
      cf = CustomField.find(key.split("_")[1].to_i)
      condition_json_template(cf.field_format, cf.possible_values)
    when /_on|_date/
      condition_json_template("date")
    else
      condition_json_template
    end

  end

  def condition_json_template(type = "text", datas = nil)
    bool_array = [["true", l('general_text_Yes')], ["false", l('general_text_No')]]
    if type == "bool"
      datas = bool_array
    end
    condition_json = {:type => type, :datas => datas}
  end

  def show_issue_status_history(issue)
  ...
  def condition_column_order(column)
    retrieve_query
    available_columns = @query.available_columns
    columns = available_columns.collect {|column| {:text => column.caption, :for => column.name.to_s} if column.caption != "#"}.compact
    if column.present?
      last_columns = column.split(",")
      last_columns = last_columns.collect{|l| columns.select{|s| s[:for] == l}.first }
      columns = ( last_columns | columns )
    end
    columns
  end

  def mokuai_reasons_options_for_select(issue)
    issue.project.mokuai_reasons
  end

  def mokuai_names_options_for_select(issue)
    available_reasons = issue.project.mokuai_reasons
    reason = issue.mokuai_reason
    reason = available_reasons.first if available_reasons.exclude?(reason)
    issue.project.mokuais reason
  end

  def find_latest_logs
    target_arr = @issue.journals.where("LENGTH(notes) > 0").order(:created_on => :desc).map{|a| {created_on: a.created_on, notes: a.notes}}<< {created_on: @issue.created_on, notes: @issue.description}
    log        = {}
    
    target_arr.each do |target|
      if /【LOG地址】/i === target[:notes]
        log[:created_on] = target[:created_on].strftime('%Y-%m-%d %H:%M').to_s
        if /【LOG时间】(.+)(\n|\z)/i === target[:notes]
          log[:log_time] = DateTime.parse $1.strip rescue nil
          log[:log_time] = log[:log_time].strftime('%Y-%m-%d %H:%M').to_s if log[:log_time].present?
        end

        if /【LOG地址】(.+)(\n|\z)/i === target[:notes]
          log[:log_addr] = $1.strip
          if /见附件/ === log[:log_addr]
            files = @issue.attachments.where("filename LIKE '%.zip' OR filename LIKE '%.rar'")
            if files.size < 2
              log[:log_addr] = files.first.try(:ftp_file_url)
            else
              log_files = files.where("filename LIKE '%log%'").order(created_on: :desc)
              log[:log_addr] = log_files.first.try(:ftp_file_url)
            end
          end
        end

        log[:video_addr] = $1.strip if /【视频地址】(.+)(\n|\z)/ === target[:notes]
        break
      end
    end
    log
