...
module CustomFieldsHelper
  ...
  def custom_field_js(name, custom_value)
    cf = custom_value.custom_field
    if cf.field_format == "user"
      select2_remote(custom_field_tag_id(name, cf), @issue)
    elsif cf.field_format == "list" && cf.possible_values.size > 10
      select2_for custom_field_tag_id(name, cf)
    elsif [cf.class.xianxiang, cf.class.pinzhi].include?(cf)
      select2_for custom_field_tag_id(name, cf), nil, :data => cf.data_json(custom_value.customized)
    # elsif cf.name.include?("版本") && cf.field_format == "string"
    #   select2_remote(custom_field_tag_id(name, cf), @issue, nil, :url => search_versions_path)
    elsif cf.name == "优先级" && cf.field_format == "list" && !User.current.is_spm?(@issue.project)
      if @issue.new_record?
        remove_select_option(custom_field_tag_id(name, cf), 1)
      else
        remove_select_option(custom_field_tag_id(name, cf), cf.possible_values.reverse.index(custom_value.to_s))
      end
    end.to_s
  end
  ...
  def custom_field_tag_with_label(name, custom_value, options={})
    custom_field_label_tag(name, custom_value, options) + custom_field_tag(name, custom_value) + custom_field_js(name, custom_value)
  end
