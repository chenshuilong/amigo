class ThirdpartyVersionReleaseJob < VersionReleaseJob
  queue_as :thirdparty_release

  def perform(thirdparty_id)
    thirdparty = Thirdparty.find(thirdparty_id)

    release_thread = Thread.new { thirdparty_release(thirdparty) }
    release_thread.join
  ...
  def thirdparty_release(thirdparty)
    repos = thirdparty.release_ids.blank? ? [] : Repo.get_thirdparty_version_release(thirdparty)
    repos.each do |r|
