...
module Redmine
  module NestedSet
    module PlanNestedSet
      def self.included(base)
        base.class_eval do
          belongs_to :parent, :class_name => self.name

          before_create :add_to_nested_set
          before_update :move_in_nested_set, :if => lambda {|plan| plan.parent_id_changed? || plan.name_changed?}
      ...
      def target_lft
        ...
        if siblings_rgt
          ...
          raise "Plan id=#{id} with parent_id=#{parent_id}: parent missing or without 'lft' value" unless parent_lft
          parent_lft + 1
        else
          1
        end
      ...
      def reload_nested_set_values
        self.lft, self.rgt = Plan.where(:id => id).pluck(:lft, :rgt).first
      end
      ...
      module ClassMethods
        ...
        def rebuild_nodes(parent_id = nil)
          nodes = Plan.where(:parent_id => parent_id).where(:rgt => nil, :lft => nil).reorder(:name)
