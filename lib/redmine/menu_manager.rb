...
module Redmine
  module MenuManager
    ...
    module MenuHelper
      ...
      def display_main_menu?(project, hide_project_menu = nil)
        return false if ["issues", "version_releases"].include?(controller_name) || hide_project_menu.present?
        menu_name = project && !project.new_record? ? :project_menu : :application_menu
        Redmine::MenuManager.items(menu_name).children.present?
      end

      def render_menu(menu, project=nil, class_name=nil)
        links = []
        menu_items_for(menu, project) do |node|
          links << render_menu_node(node, project)
        end
        links.empty? ? nil : content_tag('ul', links.join.html_safe, :class => class_name)
      end
      ...
      def render_menu_node(node, project=nil)
        if node.children.present? || !node.child_menus.nil?
          ...
          return content_tag('li', render_single_menu_node(node, caption, url, selected), class: "#{'active' if selected}")
        end
      end
      ...
      def render_unattached_menu_item(menu_item, project)
        ...
        if menu_item.allowed?(User.current, project)
          options = menu_item.html_options
          options.merge!({class: 'selected'}) if url_for == url_for(menu_item.url)
          link_to(menu_item.caption, menu_item.url, options)
        end
      end
