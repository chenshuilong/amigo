    ...
    module PDF
      module IssuesPdfHelper
        ...
        def issue_to_pdf(issue, assoc={})
          ...
          left << [l(:field_tfde), issue.tfde] unless issue.disabled_core_fields.include?('tfde_id')
        ...
        def fetch_row_values(issue, query, level)
          query.inline_columns.collect do |column|
            s = if column.is_a?(QueryCustomFieldColumn)
              ...
              if column.name == :mokuai_name
                value = issue.send(:mokuai).present?? issue.send(:mokuai).name : nil
              end

              if column.name == :status_histories
                value = eval(value).map{|v| v.values.join("  ")}.join("\n")
              end
