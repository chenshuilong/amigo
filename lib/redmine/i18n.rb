...
module Redmine
  module I18n
    ...
    def time_formatter(time)
      return if time.nil?
      if time >= 1.days.ago && time.day == Date.today.day
        time.strftime('%H:%M')
      elsif time >= 2.days.ago && time.day == (Date.today-1).day
        "#{::I18n.t('label_yesterday')} #{time.strftime('%H:%M')}"
      elsif time >= 3.days.ago && time.day == (Date.today-2).day
        "#{::I18n.t('label_day_before_yesterday')} #{time.strftime('%H:%M')}"
      elsif time > 1.years.ago && time.year == Date.today.year
    ...
    def valid_languages
      ::I18n.available_locales.reject{|lang| lang.to_s.include?('.')}
    end
